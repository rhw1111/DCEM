...
export class LuyinPage implements OnInit {
    ...
    startRecord() {
        //this.directory = this._file.documentsDirectory.replace(/^file:\/\//, '');
        ...
        this.directory = this.getSystemUrl();
    ...
    uploadRecord() {
        ...
        ftObj.upload(this.filePath.replace(/^file:\/\//, ''),
            encodeURI("http://106.15.138.75:8082/dcem/Api/Files/Upload"), options).then(
                (data) => {
                    this.base64File = "http://106.15.138.75:8082/dcem/FilesDir/" + this.fileName;
                    this.voice();
    ...
    voice() {
        ...
        this._http.postCustom(
            "http://106.15.138.75:8082/ocr/api/ocr/Voice",
            json,
            (res: any) => {
                console.log(res);
    ...
    getSystemUrl() {
        ...
        var url = "";
        if (isAndroid == true) {
            url = this._file.externalApplicationStorageDirectory;
        }
        if (isiOS == true) {
            url = this._file.documentsDirectory.replace(/^file:\/\//, '');
        }
        return url;
    }
    //插入logcall
