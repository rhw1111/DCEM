...
namespace MSLibrary.SystemToken.DAL
{
    ...
    /// 客户端系统登陆终结点数据操作
    /// </summary>
    [Injection(InterfaceType = typeof(IClientSystemLoginEndpointStore), Scope = InjectionScope.Singleton)]
    public class ClientSystemLoginEndpointStore : IClientSystemLoginEndpointStore
    {
        private ISystemTokenConnectionFactory _dbConnectionFactory;

        public ClientSystemLoginEndpointStore(ISystemTokenConnectionFactory dbConnectionFactory)
        {
        ...
        public async Task Add(ClientSystemLoginEndpoint endpoint)
        {
            await DBTransactionHelper.SqlTransactionWorkAsync(DBTypes.SqlServer, false, false, _dbConnectionFactory.CreateAllForSystemToken(), async (conn, transaction) =>
            {
                ...
                })
                {
                    ...
                    {
                        command.CommandText = @"INSERT INTO [dbo].[ClientSystemLoginEndpoint]
                                                       ([id]
                                                       ,[name]
                                                       ,[signaturekey]
                                                       ,[createtime]
                                                 ...
                                                 VALUES
                                                       ...
                                                       ,GETUTCDATE()
                                                       ,GETUTCDATE());
                                                SELECT @newid = id FROM [dbo].[ClientSystemLoginEndpoint] WHERE [sequence] = SCOPE_IDENTITY();";
                        ...
                        catch (SqlException ex)
                        {
                            ...
                            if (ex.Number == 2601)
                            {
                                var fragment = new TextFragment()
                                {
                                    ...
                                    DefaultFormatting = "客户端系统登陆终结点中存在相同的名称\"{0}\"数据",
                                    ReplaceParameters = new List<object>() { endpoint.Name }
                                };

                                throw new UtilityException((int)Errors.ExistClientSystemLoginEndpointByName, fragment);
                            }
                            else
                            {
                                throw;
        ...
        public async Task Update(ClientSystemLoginEndpoint endpoint)
        {
            await DBTransactionHelper.SqlTransactionWorkAsync(DBTypes.SqlServer, false, false, _dbConnectionFactory.CreateAllForSystemToken(), async (conn, transaction) =>
            {
                ...
                {
                    command.CommandText = @"UPDATE [dbo].[ClientSystemLoginEndpoint]
                                               SET 
                                                  [name] =@name
        ...
        public async Task Delete(Guid id)
        {
            await DBTransactionHelper.SqlTransactionWorkAsync(DBTypes.SqlServer, false, false, _dbConnectionFactory.CreateAllForSystemToken(), async (conn, transaction) =>
            {
                ...
                using (SqlCommand command = new SqlCommand()
                {
                    ...
                    CommandType = CommandType.Text,
                    Transaction = sqlTran,
                    CommandText = @"DELETE FROM [dbo].[ClientSystemLoginEndpoint] WHERE id=@id;"
                })
                {
        ...
        public async Task<ClientSystemLoginEndpoint> QueryByName(string name)
        {
            ...
            await DBTransactionHelper.SqlTransactionWorkAsync(DBTypes.SqlServer, true, false, _dbConnectionFactory.CreateReadForSystemToken(), async (conn, transaction) =>
            {
                ...
                using (SqlCommand command = new SqlCommand()
                {
                    Connection = (SqlConnection)conn,
                    CommandType = CommandType.Text,
                    CommandText = string.Format(@"SELECT {0} FROM [dbo].[ClientSystemLoginEndpoint] WHERE name=@name;", StoreHelper.GetClientSystemLoginEndpointSelectFields(string.Empty)),
                    Transaction = sqlTran
                })
                {
        ...
        {
            QueryResult<ClientSystemLoginEndpoint> result = new QueryResult<ClientSystemLoginEndpoint>();
            await DBTransactionHelper.SqlTransactionWorkAsync(DBTypes.SqlServer, true, false, _dbConnectionFactory.CreateReadForSystemToken(), async (conn, transaction) =>
            {
                SqlTransaction sqlTran = null;
                if (transaction != null)
                ...
                using (SqlCommand command = new SqlCommand()
                {
                    ...
                    CommandText = string.Format(@"SET @currentpage = @page;
                                                    SELECT @count = COUNT(*)
                                                    FROM [ClientSystemLoginEndpoint]
                                                    ...
                                                    StoreHelper.GetClientSystemLoginEndpointSelectFields(string.Empty)),
                    Transaction = sqlTran
                })
                {
                    ...
                    using (reader = await command.ExecuteReaderAsync())
                    {
                        ...
                        {
                            var endpoint = new ClientSystemLoginEndpoint();
