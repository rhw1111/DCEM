...
{
    /// <summary>
    /// 针对ADFSPassword模式的令牌生成
    /// 需要传入的参数：
    /// AdfsUrl:string,ADFS的地址
    /// CrmUrl:string,Crm的地址
    /// ClientId:string,在ADFS中注册的服务器应用程序的ClientId
    /// ClientSecret:string,在ADFS中注册的服务器应用程序的密码
    /// UserName:string,用户名
    /// Password:string,密码
    /// </summary>
    [Injection(InterfaceType = typeof(CrmServiceTokenGenerateServiceForADFSPassword), Scope = InjectionScope.Singleton)]
    public class CrmServiceTokenGenerateServiceForADFSPassword : ICrmServiceTokenGenerateService
    {
        ...
        public async Task<string> Genereate(Dictionary<string, object> parameters)
        {
            ...
            if (!(parameters[CrmServiceTokenGenerateServiceParameterNames.ClientSecret] is string))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.ParameterTypeNotMatchInCrmServiceTokenGenerateService,
                    DefaultFormatting = "在Crm服务令牌生成服务{0}中，名称为{1}的参数期望类型为{2}，而实际类型为{3}",
                    ReplaceParameters = new List<object>() { this.GetType().FullName, CrmServiceTokenGenerateServiceParameterNames.ClientSecret, typeof(string).FullName, parameters[CrmServiceTokenGenerateServiceParameterNames.ClientSecret].GetType().FullName }
                };

                throw new UtilityException((int)Errors.ParameterTypeNotMatchInCrmServiceTokenGenerateService,fragment);
            }
            string strClientSecret = parameters[CrmServiceTokenGenerateServiceParameterNames.ClientSecret].ToString();
            ...
            await GetAdfsAuth(strAdfsUrl, strClientId, strClientSecret, strCrmUrl, strUserName, strPassword, async (adfsAuth) =>
            {
        ...
        {
            return JsonSerializerHelper.Serializer(new AdfsParameter() { AdfsUrl = adfsUrl, ClientId = clientId, CrmUrl = crmUrl, ClientSecret = clientSecret, UserName = userName, Password = password });
        }
        ...
        private async Task GetAdfsAuth(string adfsUrl, string clientId, string clientSecret, string crmUrl, string userName, string password, Func<AdfsAuth, Task> action)
        {
            ...
            if (!_adfsAuthContexts.TryGetValue(strKey, out AdfsAuthWrapperContainer wrapperContainer))
            {
                ...
                async () =>
                {
                    var auth = await AdfsHelper.GetAdfsAuthDirect(adfsUrl, crmUrl, clientId, clientSecret, userName, password);
                    AdfsAuthWrapper wrapper = new AdfsAuthWrapper()
                    {
                        AdfsAuth = auth,
                        AdfsParameter = new AdfsParameter() { AdfsUrl = adfsUrl, ClientId = clientId, CrmUrl = crmUrl,  ClientSecret = clientSecret, UserName = userName, Password = password },
