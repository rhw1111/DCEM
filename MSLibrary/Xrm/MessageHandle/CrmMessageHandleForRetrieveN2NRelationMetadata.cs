...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveN2NRelationMetadata), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveN2NRelationMetadata : ICrmMessageHandle
    {
        public async Task<CrmRequestMessageHandleResult> ExecuteRequest(CrmRequestMessage request)
        {
            if (!(request is CrmRetrieveN2NRelationMetadataRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveN2NRelationMetadataRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmRetrieveN2NRelationMetadataRequestMessage;
            ...
            url = $"{url}/Microsoft.Dynamics.CRM.ManyToManyRelationshipMetadata";
