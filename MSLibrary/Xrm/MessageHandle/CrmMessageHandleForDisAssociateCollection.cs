...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForDisAssociateCollection), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForDisAssociateCollection : ICrmMessageHandle
    {
        private ICrmAlternateKeyTypeHandle _crmAlternateKeyTypeHandle;
        public CrmMessageHandleForDisAssociateCollection(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle)
        {
        ...
        {
            if (!(request is CrmDisAssociateCollectionRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmDisAssociateCollectionRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmDisAssociateCollectionRequestMessage;
            ...
            url = $"{url}/{ realRequest.AttributeName}({realRequest.DisAssociateEntityId.ToString()})/$ref";
        ...
        {
            CrmDisAssociateCollectionResponseMessage response = new CrmDisAssociateCollectionResponseMessage();
