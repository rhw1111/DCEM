...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForUpsert), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForUpsert : ICrmMessageHandle
    {
        ...
        public CrmMessageHandleForUpsert(ICrmExecuteEntityConvertJObjectService crmExecuteEntityConvertJObjectService, ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle)
        {
        ...
        {
            if (!(request is CrmUpsertRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmUpsertRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch,fragment);
            }

            var realRequest = request as CrmUpsertRequestMessage;
            //获取实体记录的json对象
