...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveRelationMetadata), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveRelationMetadata : ICrmMessageHandle
    {
        public async Task<CrmRequestMessageHandleResult> ExecuteRequest(CrmRequestMessage request)
        {
            if (!(request is CrmRetrieveRelationMetadataRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveRelationMetadataRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmRetrieveRelationMetadataRequestMessage;
        ...
        {
            CrmRetrieveRelationMetadataResponseMessage response = new CrmRetrieveRelationMetadataResponseMessage();
