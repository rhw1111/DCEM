...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForDisAssociateLookup), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForDisAssociateLookup : ICrmMessageHandle
    {
        private ICrmAlternateKeyTypeHandle _crmAlternateKeyTypeHandle;
        public CrmMessageHandleForDisAssociateLookup(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle)
        {
        ...
        {
            if (!(request is CrmDisAssociateLookupRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { "MSLibrary.SystemToken.Application.AppRefreashCommonToken", typeof(CrmDisAssociateLookupRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmDisAssociateLookupRequestMessage;
        ...
        {
            CrmDisAssociateLookupResponseMessage response = new CrmDisAssociateLookupResponseMessage();
