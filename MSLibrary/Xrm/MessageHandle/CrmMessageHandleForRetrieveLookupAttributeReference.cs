...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveLookupAttributeReference), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveLookupAttributeReference : ICrmMessageHandle
    {
        ...
        public CrmMessageHandleForRetrieveLookupAttributeReference(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle, ICrmRetrieveJTokenConvertService crmRetrieveJTokenConvertService)
        {
        ...
        public async Task<CrmRequestMessageHandleResult> ExecuteRequest(CrmRequestMessage request)
        {
            if (!(request is CrmRetrieveLookupAttributeReferenceRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveLookupAttributeReferenceRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };
        ...
        {
            var request = (CrmRetrieveLookupAttributeReferenceRequestMessage)extension;

            CrmRetrieveLookupAttributeReferenceResponseMessage response = new CrmRetrieveLookupAttributeReferenceResponseMessage();

            var jObject = JsonSerializerHelper.Deserialize<JObject>(responseBody);

            var entityReference = await _crmRetrieveJTokenConvertService.Convert<CrmEntityReference>(jObject["value"]);

            response.Value = entityReference;
