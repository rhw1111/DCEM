...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForAssociateLookup), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForAssociateLookup : ICrmMessageHandle
    {
        private ICrmAlternateKeyTypeHandle _crmAlternateKeyTypeHandle;
        public CrmMessageHandleForAssociateLookup(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle)
        {
        ...
        {
            if (!(request is CrmAssociateLookupRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmAssociateLookupRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };
        ...
        {
            CrmAssociateLookupResponseMessage response = new CrmAssociateLookupResponseMessage();
