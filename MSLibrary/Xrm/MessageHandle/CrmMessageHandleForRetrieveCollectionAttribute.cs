...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveCollectionAttribute), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveCollectionAttribute : ICrmMessageHandle
    {
        ...
        public CrmMessageHandleForRetrieveCollectionAttribute(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle, ICrmRetrieveJTokenConvertService crmRetrieveJTokenConvertService)
        {
        ...
        {
            if (!(request is CrmRetrieveCollectionAttributeRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveCollectionAttributeRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmRetrieveCollectionAttributeRequestMessage;
        ...
        {
            var request = (CrmRetrieveCollectionAttributeRequestMessage)extension;

            CrmRetrieveCollectionAttributeResponseMessage response = new CrmRetrieveCollectionAttributeResponseMessage();
