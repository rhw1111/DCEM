...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveLookupAttribute), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveLookupAttribute : ICrmMessageHandle
    {
        ...
        public CrmMessageHandleForRetrieveLookupAttribute(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle, ICrmRetrieveJTokenConvertService crmRetrieveJTokenConvertService)
        {
        ...
        {
            if (!(request is CrmRetrieveLookupAttributeRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveLookupAttributeRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmRetrieveLookupAttributeRequestMessage;
            ...
            url = $"{url}/{realRequest.AttributeName}?{realRequest.QueryExpression}";
        ...
        {
            var request = (CrmRetrieveLookupAttributeRequestMessage)extension;

            CrmRetrieveLookupAttributeResponseMessage response = new CrmRetrieveLookupAttributeResponseMessage();

            var jObject = JsonSerializerHelper.Deserialize<JObject>(responseBody);

            var entity=await _crmRetrieveJTokenConvertService.Convert<CrmEntity>(jObject,new Dictionary<string, object>() { { CrmRetrieveJTokenHandleExtensionParameterNames.EntityName,request.EntityName} });
