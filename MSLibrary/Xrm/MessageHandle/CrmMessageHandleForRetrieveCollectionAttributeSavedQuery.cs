...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveCollectionAttributeSavedQuery), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveCollectionAttributeSavedQuery : ICrmMessageHandle
    {
        ...
        public CrmMessageHandleForRetrieveCollectionAttributeSavedQuery(ICrmAlternateKeyTypeHandle crmAlternateKeyTypeHandle,ICrmRetrieveJTokenConvertService crmRetrieveJTokenConvertService)
        {
        ...
        {
            if (!(request is CrmRetrieveCollectionAttributeSavedQueryRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveCollectionAttributeSavedQueryRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };

                throw new UtilityException((int)Errors.CrmRequestMessageTypeNotMatch, fragment);
            }

            var realRequest = request as CrmRetrieveCollectionAttributeSavedQueryRequestMessage;
            ...
            url = $"{url}?savedQuery={realRequest.SavedQueryId.ToString()}";
        ...
        {
            var request = (CrmRetrieveCollectionAttributeSavedQueryRequestMessage)extension;

            CrmRetrieveCollectionAttributeSavedQueryResponseMessage response = new CrmRetrieveCollectionAttributeSavedQueryResponseMessage();
