...
{
    [Injection(InterfaceType = typeof(CrmMessageHandleForRetrieveEntityMetadata), Scope = InjectionScope.Singleton)]
    public class CrmMessageHandleForRetrieveEntityMetadata : ICrmMessageHandle
    {
        public async Task<CrmRequestMessageHandleResult> ExecuteRequest(CrmRequestMessage request)
        {
            if (!(request is CrmRetrieveEntityMetadataRequestMessage))
            {
                var fragment = new TextFragment()
                {
                    Code = TextCodes.CrmRequestMessageTypeNotMatch,
                    DefaultFormatting = "消息请求类型不匹配，期待的类型为{0}，实际类型为{1}，位置为{2}",
                    ReplaceParameters = new List<object>() { typeof(CrmRetrieveEntityMetadataRequestMessage).FullName, request.GetType().FullName, $"{ this.GetType().FullName }.ExecuteRequest" }
                };
        ...
        {
            CrmRetrieveEntityMetadataResponseMessage response = new CrmRetrieveEntityMetadataResponseMessage();
            ...
            response.EntityMetadata = results[0];
