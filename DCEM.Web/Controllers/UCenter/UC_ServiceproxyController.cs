...
{
    #region 控制器
    [Route("Api/UC_Serviceproxy")]
    [EnableCors("any")]
    [ApiController]
    public class UC_ServiceproxyController : ApiController
    {
        #region 初始化
        IServiceproxyService _serviceproxyService;
        public UC_ServiceproxyController()
        {
        ...
        [HttpGet]
        [Route("UcQueryList")]
        public async Task<NewtonsoftJsonActionResult<QueryResult<CrmEntity>>> UcQueryList(string phone, int? status,int pageindex = 1, int pagesize=10)
        {
            return await _serviceproxyService.UcQueryList(phone, status, pageindex, pagesize);
        }

        #region 查询 问诊单 服务委托书 详情--用于C端
        [HttpGet]
        [Route("UcQueryInfo")]
        public async Task<NewtonsoftJsonActionResult<ServiceproxyQueryInfoResponse>> UcQueryInfo(string guid)
        {
            return await _serviceproxyService.UcQueryInfo(guid);
        }
        #endregion
        #endregion
